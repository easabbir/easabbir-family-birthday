    ...
    <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-8 relative">
      ...
      <div className="relative w-full h-12 mb-4">
        ...
        {monthMarkers.map((m) => (
          ...
          >
            <span className="absolute top-4 -translate-x-1/2 text-[9px] font-mono text-slate-400 uppercase tracking-widest">
              {m.label}
            </span>
          </div>
        ))}
        ...
        >
          <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">
            TODAY
          </span>
        </div>
        ...
        {markers.map((marker) => {
          ...
          return (
            ...
            >
              <div 
                className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[10px] text-white font-mono shadow-sm overflow-hidden border-2 ${marker.isPast ? 'border-slate-300 opacity-60' : 'border-white ring-2 ring-slate-100'}`}
              ...
              {/* Tooltip */}
              <div className="absolute top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-[10px] py-1 px-2 rounded whitespace-nowrap pointer-events-none">
                {marker.member.name}
              </div>
            </div>
          );
