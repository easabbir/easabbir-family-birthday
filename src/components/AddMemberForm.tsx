...
export const AddMemberForm: React.FC<Props> = ({ initialData, onSave, onClose }) => {
  ...
  return (
    <div className="glass-panel-heavy p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden w-full max-w-md border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
      <div className="absolute top-0 left-0 w-full h-1.5 bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500"></div>
      
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white drop-shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex items-center gap-5 glass-panel p-4 rounded-2xl border-white/20 dark:border-white/5 bg-slate-100/50 dark:bg-black/10">
          ...
          <div className="flex-1">
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-1">Profile Photo</label>
            <p className="text-[10px] text-slate-500 dark:text-slate-500">Tap to upload (Max 2MB)</p>
          </div>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-2 ml-1">Full Name</label>
            <input
              type="text" required value={name} onChange={(e) => setName(e.target.value)}
              className="w-full px-4 py-3 glass-panel rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none transition-all text-slate-900 dark:text-white placeholder-slate-400"
              placeholder="e.g. John Smith"
            />
          </div>

          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-2 ml-1">Relationship</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-2 ml-1">Birth Date</label>
              <input
                type="date" required max={new Date().toISOString().split('T')[0]} value={dobDate} onChange={(e) => setDobDate(e.target.value)}
                className="w-full px-4 py-3 glass-panel rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none transition-all text-slate-900 dark:text-white text-sm"
              />
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-2 ml-1">Time (Optional)</label>
              <input
